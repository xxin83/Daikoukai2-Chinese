...
class AdvancedRomTranslatorApp:
    ...
    def create_widgets(self):
        ...
        ttk.Button(file_frame, text="Load Extracted Dump (.txt)", command=self.load_extracted_dump).grid(row=3, column=0, padx=5, pady=3, sticky="w")
        self.lbl_dump = ttk.Label(file_frame, text="Not loaded", foreground="gray")
        self.lbl_dump.grid(row=3, column=1, padx=10, pady=3, sticky="w")

        self.cmb_dump_addr = ttk.Combobox(file_frame, state="readonly", width=42)
        self.cmb_dump_addr.grid(row=4, column=0, padx=5, pady=3, sticky="w")
        self.cmb_dump_addr.bind("<<ComboboxSelected>>", self.on_dump_address_selected)
        self.btn_load_addr = ttk.Button(file_frame, text="Load Selected Address Content", command=self.load_selected_dump_block, state="disabled")
        self.btn_load_addr.grid(row=4, column=1, padx=10, pady=3, sticky="w")
    ...
    # ==================== 核心逻辑：智能无损分句机制 ====================
    def parse_extracted_dump(self, filepath):
        """Parse [Block xxxx] ... $ADDR blocks from extracted txt."""
        blocks = []
        current_block = None
        current_hex_tokens = []
        block_re = re.compile(r'^\[Block\s+(\d+)\].*?\$([0-9A-Fa-f]+)')
        ...
        with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
            for raw_line in f:
                ...
                if m:
                    if current_block and current_hex_tokens:
                        current_block["hex"] = "".join(current_hex_tokens).upper()
                ...
                if current_block and line and hex_line_re.match(line):
                    tokens = re.findall(r'[0-9A-Fa-f]{2}', line)
                    current_hex_tokens.extend(tokens)
    ...
    def load_extracted_dump(self):
        path = filedialog.askopenfilename(filetypes=[("Text Files", "*.txt"), ("All Files", "*.*")])
        if not path:
            return
        try:
            blocks = self.parse_extracted_dump(path)
            if not blocks:
                messagebox.showwarning("鎻愮ず", "No valid [Block ... $ADDR] records found.")
                return

            self.dump_blocks = blocks
            display_values = [
                f"Block {b['block_id']} | ${b['address']} | {len(b['hex']) // 2} bytes"
                for b in self.dump_blocks
            ]
            self.cmb_dump_addr["values"] = display_values
            self.cmb_dump_addr.current(0)
            self.btn_load_addr.config(state="normal")
            self.lbl_dump.config(
                text=f"Loaded {len(self.dump_blocks)} blocks - {os.path.basename(path)}",
                foreground="green"
            )
            self.load_selected_dump_block()
        except Exception as e:
            messagebox.showerror("閿欒", f"Failed to parse dump file: {str(e)}")

    def on_dump_address_selected(self, _event=None):
        self.load_selected_dump_block()

    def load_selected_dump_block(self):
        idx = self.cmb_dump_addr.current()
        if idx < 0 or idx >= len(self.dump_blocks):
            return

        block_hex = self.dump_blocks[idx]["hex"]
        if not block_hex:
            messagebox.showwarning("鎻愮ず", "Selected block has no hex data.")
            return

        formatted = " ".join(block_hex[i:i+2] for i in range(0, len(block_hex), 2))
        self.txt_box1_full_hex.delete("1.0", tk.END)
        self.txt_box1_full_hex.insert("1.0", formatted)
        self.analyze_and_split_hex()
